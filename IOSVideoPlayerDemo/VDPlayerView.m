...
@interface VDPlayerView () <UIGestureRecognizerDelegate,UIAlertViewDelegate>
...
/** 播发器的几种状态 */
@property (nonatomic, assign) VDPlayerState          state;
/** 是否为全屏 */
...
/** 亮度view */
@property (nonatomic, strong) VDBrightnessView       *brightnessView;
/** 视频填充模式 */
...
- (void)dealloc {
    self.playerItem = nil;
    self.scrollView  = nil;
    VDPlayerShared.isLockScreen = NO;
    [self.controlView vd_playerCancelAutoFadeOutControlView];
    // 移除通知
...
+ (instancetype)sharedPlayerView {
    static VDPlayerView *playerView = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        playerView = [[VDPlayerView alloc] init];
    });
    return playerView;
}
...
- (void)playerControlView:(UIView *)controlView playerModel:(VDPlayerModel *)playerModel {
    if (!controlView) {
        // 指定默认控制层
        VDPlayerControlView *defaultControlView = [[VDPlayerControlView alloc] init];
...
- (void)resetPlayer {
    ...
    if (self.isChangeResolution) { // 切换分辨率
        [self.controlView vd_playerResetControlViewForResolution];
        self.isChangeResolution = NO;
    }else { // 重置控制层View
        [self.controlView vd_playerResetControlView];
    }
 ...
 */
- (void)play {
    [self.controlView vd_playerPlayBtnState:YES];
    if (self.state == VDPlayerStatePause) { self.state = VDPlayerStatePlaying; }
 ...
 */
- (void)pause {
    [self.controlView vd_playerPlayBtnState:NO];
    if (self.state == VDPlayerStatePlaying) { self.state = VDPlayerStatePause;}
...
- (void)cellVideoWithScrollView:(UIScrollView *)scrollView
    ...
    // 在cell播放
    [self.controlView vd_playerCellPlay];
...
- (void)configZFPlayer {
    ...
    if ([self.videoURL.scheme isEqualToString:@"file"]) {
        self.state = VDPlayerStatePlaying;
        self.isLocalVideo = YES;
        [self.controlView vd_playerDownloadBtnState:NO];
    } else {
        self.state = VDPlayerStateBuffering;
        self.isLocalVideo = NO;
        [self.controlView vd_playerDownloadBtnState:YES];
    }
    // 开始播放
    [self play];
    self.isPauseByUser = NO;
...
- (void)createTimer {
    ...
    self.timeObserve = [self.player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(1, 1) queue:nil usingBlock:^(CMTime time){
        ...
        if (loadedRanges.count > 0 && currentItem.duration.timescale != 0) {
            ...
            [weakSelf.controlView vd_playerCurrentTime:currentTime totalTime:totalTime sliderValue:value];
        }
    }];
}
    ...
    if (object == self.player.currentItem) {
        if ([keyPath isEqualToString:@"status"]) {
            ...
            if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
                ...
                self.state = VDPlayerStateFailed;
            }
            ...
            [self.controlView vd_playerSetProgress:timeInterval / totalDuration];
            ...
            // 当缓冲好的时候
            if (self.playerItem.playbackLikelyToKeepUp && self.state == VDPlayerStateBuffering){
                self.state = VDPlayerStatePlaying;
            }
        }
    } else if (object == self.scrollView) {
        if ([keyPath isEqualToString:kVDPlayerViewContentOffset]) {
...
- (void)updatePlayerViewToBottom {
    ...
    // 小屏播放
    [self.controlView vd_playerBottomShrinkPlay];
}
...
- (void)toOrientation:(UIInterfaceOrientation)orientation {
    ...
    if (orientation != UIInterfaceOrientationPortrait) {//
        ...
        if (currentOrientation == UIInterfaceOrientationPortrait) {
            [self removeFromSuperview];
            VDBrightnessView *brightnessView = [VDBrightnessView sharedBrightnessView];
...
- (void)lockScreenAction:(UIButton *)sender {
    ...
    VDPlayerShared.isLockScreen = sender.selected;
}
...
- (void)unLockTheScreen {
    ...
    VDPlayerShared.isLockScreen = NO;
    [self.controlView vd_playerLockBtnState:NO];
    self.isLocked = NO;
    ...
    if (gesture.state == UIGestureRecognizerStateRecognized) {
        if (self.isBottomVideo && !self.isFullScreen) { [self _fullScreenAction]; }
            ...
            if (self.playDidEnd) { return; }
                ...
                [self.controlView vd_playerShowOrHideControlView];
            }
        }
    }
}
...
- (void)doubleTapAction:(UIGestureRecognizer *)gesture {
    if (self.playDidEnd) { return;  }
    // 显示控制层
    [self.controlView vd_playerShowControlView];
...
- (void)shrikPanAction:(UIPanGestureRecognizer *)gesture {
    ...
    VDPlayerView *view = (VDPlayerView *)gesture.view;
...
- (void)_fullScreenAction {
    if (VDPlayerShared.isLockScreen) {
...
- (void)moviePlayDidEnd:(NSNotification *)notification {
    self.state = VDPlayerStateStopped;
    if (self.isBottomVideo && !self.isFullScreen) { // 播放完了，如果是在小屏模式 && 在bottom位置，直接关闭播放器
        ...
        if (!self.isDragged) { // 如果不是拖拽中，直接结束播放
            self.playDidEnd = YES;
            [self.controlView vd_playerPlayEnd];
        }
    }
}
...
- (void)appDidEnterBackground {
    ...
    // 退到后台锁定屏幕方向
    VDPlayerShared.isLockScreen = YES;
    [_player pause];
    self.state                  = VDPlayerStatePause;
}
...
- (void)appDidEnterPlayground {
    ...
    VDPlayerShared.isLockScreen = self.isLocked;
    if (!self.isPauseByUser) {
        self.state         = VDPlayerStatePlaying;
        self.isPauseByUser = NO;
        [self play];
...
- (void)seekToTime:(NSInteger)dragedSeconds completionHandler:(void (^)(BOOL finished))completionHandler {
    if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
        ...
        [self.controlView vd_playerActivity:YES];
        [self.player pause];
        ...
        [self.player seekToTime:dragedCMTime toleranceBefore:CMTimeMake(1,1) toleranceAfter:CMTimeMake(1,1) completionHandler:^(BOOL finished) {
            [weakSelf.controlView vd_playerActivity:NO];
            // 视频跳转回调
            ...
            // 结束滑动
            [weakSelf.controlView vd_playerDraggedEnd];
            if (!weakSelf.playerItem.isPlaybackLikelyToKeepUp && !weakSelf.isLocalVideo) { weakSelf.state = VDPlayerStateBuffering; }
...
- (void)horizontalMoved:(CGFloat)value {
    ...
    self.isDragged = YES;
    [self.controlView vd_playerDraggedTime:self.sumTime totalTime:totalMovieDuration isForward:style hasPreview:NO];
}
 ...
 */
- (void)setState:(VDPlayerState)state {
    _state = state;
    // 控制菊花显示、隐藏
    [self.controlView vd_playerActivity:state == VDPlayerStateBuffering];
    if (state == VDPlayerStatePlaying || state == VDPlayerStateBuffering) {
        // 隐藏占位图
        [self.controlView vd_playerItemPlaying];
    } else if (state == VDPlayerStateFailed) {
        NSError *error = [self.playerItem error];
        [self.controlView vd_playerItemStatusFailed:error];
    }
}
...
- (void)setScrollView:(UIScrollView *)scrollView {
    ...
    if (_scrollView) {
        [_scrollView removeObserver:self forKeyPath:kVDPlayerViewContentOffset];
    }
    _scrollView = scrollView;
    if (scrollView) { [scrollView addObserver:self forKeyPath:kVDPlayerViewContentOffset options:NSKeyValueObservingOptionNew context:nil]; }
}
 ...
 */
- (void)setPlayerLayerGravity:(VDPlayerLayerGravity)playerLayerGravity {
    _playerLayerGravity = playerLayerGravity;
    switch (playerLayerGravity) {
        case VDPlayerLayerGravityResize:
            self.playerLayer.videoGravity = AVLayerVideoGravityResize;
            self.videoGravity = AVLayerVideoGravityResize;
            break;
        case VDPlayerLayerGravityResizeAspect:
            self.playerLayer.videoGravity = AVLayerVideoGravityResizeAspect;
            self.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case VDPlayerLayerGravityResizeAspectFill:
...
- (void)setHasDownload:(BOOL)hasDownload {
    ...
    [self.controlView vd_playerHasDownloadFunction:hasDownload];
}
...
- (void)setPlayerModel:(VDPlayerModel *)playerModel {
    _playerModel = playerModel;
    
    if (playerModel.seekTime) { self.seekTime = playerModel.seekTime; }
    [self.controlView vd_playerModel:playerModel];
    // 分辨率
...
- (VDBrightnessView *)brightnessView {
    if (!_brightnessView) {
        _brightnessView = [VDBrightnessView sharedBrightnessView];
    }
...
- (void)zf_controlView:(UIView *)controlView backAction:(UIButton *)sender {
    if (VDPlayerShared.isLockScreen) {
        ...
        if (!self.isFullScreen) {
            ...
            [self pause];
            if ([self.delegate respondsToSelector:@selector(vd_playerBackAction)]) { [self.delegate vd_playerBackAction]; }
        } else {
            [self interfaceOrientation:UIInterfaceOrientationPortrait];
        }
    }
...
- (void)zf_controlView:(UIView *)controlView downloadVideoAction:(UIButton *)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(vd_playerDownload:)]) {
        [self.delegate vd_playerDownload:urlStr];
    }
}
...
- (void)zf_controlView:(UIView *)controlView progressSliderValueChanged:(UISlider *)slider {
    ...
    if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
        ...
        [controlView vd_playerDraggedTime:dragedSeconds totalTime:totalTime isForward:style hasPreview:self.isFullScreen ? self.hasPreviewView : NO];
                ...
                AVAssetImageGeneratorCompletionHandler handler = ^(CMTime requestedTime, CGImageRef im, CMTime actualTime, AVAssetImageGeneratorResult result, NSError *error){
                    ...
                    if (result != AVAssetImageGeneratorSucceeded) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [controlView vd_playerDraggedTime:dragedSeconds sliderImage:self.thumbImg ? : VDPlayerImage(@"ZFPlayer_loading_bgView")];
                        });
                    } else {
...
- (void)zf_controlViewWillShow:(UIView *)controlView isFullscreen:(BOOL)fullscreen {
    if ([self.delegate respondsToSelector:@selector(vd_playerControlViewWillShow:isFullscreen:)]) {
        [self.delegate vd_playerControlViewWillShow:controlView isFullscreen:fullscreen];
    }
}

- (void)zf_controlViewWillHidden:(UIView *)controlView isFullscreen:(BOOL)fullscreen {
    if ([self.delegate respondsToSelector:@selector(vd_playerControlViewWillHidden:isFullscreen:)]) {
        [self.delegate vd_playerControlViewWillHidden:controlView isFullscreen:fullscreen];
    }
}
